...
Loading a shipped profile replaces a recognized installation-owned predecessor bundle list with the current template before resolving bundle patches. The loader matches the complete ordered list and leaves every other list unchanged, so a customized profile is never silently rewritten.
