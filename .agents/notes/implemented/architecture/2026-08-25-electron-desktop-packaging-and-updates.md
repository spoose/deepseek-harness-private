...
Local macOS, Windows, and Linux ARM64 installation testing uses an explicit `--unsigned` package invocation with the same build and runtime preparation. It strips certificate inputs, isolates artifacts in `unsigned-artifacts`, and omits updater configuration and the release completion record. macOS uses ad-hoc signatures without hardened runtime or notarization; native files are signed before the runtime inventory is sealed so application assembly preserves the recorded bytes. Linux ARM64 emits a DEB only on a native Linux ARM64 host and has no upload command. The regular macOS and Windows package commands explicitly select signed mode even when their parent environment requests unsigned mode. This separation permits local use without Apple credentials or an EV token while preventing local test output from qualifying for release upload.
